...
import { 
  ...
  RefreshCw, Wifi, Key, Lock, ShieldCheck, Database, HelpCircle 
} from 'lucide-react';
...
export function ApiHealthMonitor() {
  ...
  useEffect(() => {
    const handleApiEvent = (e: Event) => {
      ...
      // If an API request fails, automatically trigger the beautiful AI Status Overlay
    ...
    return () => {
      window.removeEventListener('api-health-event', handleApiEvent);
    };
  }, []);
  ...
  return (
    <>
      {/* Mini status widget in Top Header */}
      <div 
      ...
      {/* Main AI Status Overlay triggered on Errors */}
      {showOverlay && (
        <div className="fixed inset-0 z-[350] flex items-center justify-center p-4 bg-slate-950/85 backdrop-blur-md animate-fade-in RTL" style={{ direction: 'rtl' }}>
          <div className="w-full max-w-xl bg-slate-900 border border-rose-900/40 rounded-2xl shadow-2xl overflow-hidden flex flex-col text-slate-100 animate-in zoom-in-95 duration-200">
            ...
            <div className="p-6 space-y-5 overflow-y-auto max-h-[70vh]">
              {/* Context Summary */}
              <div className="p-3 bg-rose-950/20 rounded-xl border border-rose-900/20 text-xs text-rose-200 leading-relaxed space-y-1">
      ...
      {/* Traditional Side-Drawer for Logs Detail View */}
      {isOpen && (
